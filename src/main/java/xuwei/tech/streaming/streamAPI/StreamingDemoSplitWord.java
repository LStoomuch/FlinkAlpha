 ...
 */
public class StreamingDemoSplitWord {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //选择一个或者多个切分后的流
        DataStream<WordWithCount> evenStream = splitStream.select("c").flatMap(new FlatMapFunction<Agg, WordWithCount>() {
            @Override
        ...
        DataStream<WordWithCount> oddStream = splitStream.select("d").flatMap(new FlatMapFunction<Agg, WordWithCount>() {
            @Override
            public void flatMap(Agg agg, Collector<WordWithCount> collector) throws Exception {
                System.out.println("key:" + agg.getKey() + "  type:" + agg.getType());
                collector.collect(new WordWithCount(agg.getKey(), agg.getType(), 1));
        ...
        String jobName = StreamingDemoSplitWord.class.getSimpleName();
        env.execute(jobName);
    }
